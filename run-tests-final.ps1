...
# Run the tests
Invoke-Pester "$PSScriptRoot/tests/auto-detection" -OutputFormat LegacyNUnitXml -OutputFile "$PSScriptRoot/test-results.xml" -PassThru | ForEach-Object {
    Write-Host "Tests completed with $($this.FailedCount) failed tests"
}